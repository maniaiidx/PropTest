...
namespace Mebiustos.BlendShapeEditHelper
{
    ...
    public class BlendShapeEditHelperEditor : Editor
    {
        ...
        void DrawButtons()
        {
            ...
            //■MJで追加//////////////
            if (GUILayout.Button("再生中に編集モード", GUILayout.ExpandWidth(false)))
            {
                DataCounter DC = GameObject.Find("Server").GetComponent<DataCounter>();
                BreathController.OriBreathController
                    OriBC = DC.GirlTrs.GetComponent<BreathController.OriBreathController>();

                if (DC.girlAnim.updateMode == AnimatorUpdateMode.AnimatePhysics)
                {
                    DC.girlAnim.updateMode = AnimatorUpdateMode.Normal;
                    OriBC.enabled = true;
                }
                else if (DC.girlAnim.updateMode == AnimatorUpdateMode.Normal)
                {
                    DC.girlAnim.updateMode = AnimatorUpdateMode.AnimatePhysics;
                    OriBC.enabled = false;
                }
        ...
        SkinnedMeshRenderer[] GetSkinnedMeshRenderers()
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                if (
                    cnt > 0
                    && renderers[i].name != "Body"//MJが追加した条件
                    && renderers[i].name != "Socks"//MJが追加した条件
                    && renderers[i].name != "Spats"//MJが追加した条件
                    )
                {
                    smeshList.Add(rend);
                }
