...
namespace Mebiustos.BreathController {
    [CustomEditor(typeof(OriBreathController))]
    public class OriBreathControllerEditor : Editor {

        public override void OnInspectorGUI() {
            var breath = (OriBreathController)target;
            bool change = false;
            ...
            if (change) {
                ...
                //動作中にDuration数値いじるとエラーが出るのでコメントアウト
        ...
        bool AdvancedConfig(OriBreathController breath) {
            bool change = false;
            ...
            if (this.isFoldoutAdv) {
                ...
                if (GUILayout.Button("Restore default advanced configs", GUILayout.ExpandWidth(false))) {
                    if (EditorUtility.DisplayDialog(
                        ...
                        "Restore", "Cancel")) {
                            breath.maxDuration = OriBreathController.InitialDurationInhale;
                            breath.minDuration = OriBreathController.InitialDurationExhale;
                            breath.restDuration = OriBreathController.InitialDurationRest;

                            breath.SpineInhaleAngle = OriBreathController.InitialAngleSpineInhale;
                            breath.SpineExhaleAngle = OriBreathController.InitialAngleSpineExhale;
                            breath.ChestInhaleAngle = OriBreathController.InitialAngleChestInhale;
                            breath.ChestExhaleAngle = OriBreathController.InitialAngleChestExhale;
                            breath.NeckInhaleAngle = OriBreathController.InitialAngleNeckInhale;
                            breath.NeckExhaleAngle = OriBreathController.InitialAngleNeckExhale;
                            breath.HeadInhaleAngle = OriBreathController.InitialAngleHeadInhale;
                        
                            breath.HeadExhaleAngle = OriBreathController.InitialAngleHeadExhale;
                ...
                if (this.isFoldoutAdvEtc) {
                    GUI.changed = false;
                    EditorGUI.indentLevel++;
                    breath.InhalingMethod = (OriBreathController.HalingMethod)EditorGUILayout.EnumPopup("Inhale Method", breath.InhalingMethod);
                    EditorGUI.indentLevel--;
                    change |= GUI.changed;
                }
                EditorGUI.indentLevel--;
        ...
        // Debug angle
        void xyz(OriBreathController.Angle angle) {
