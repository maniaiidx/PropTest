...
public partial class DataCounter
{
    ...
    public IEnumerator RME_citycaptured()
    {
        ...
        GameObject Ieki010_SuiiLastPosObj
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_Iekiyd_Suii002PosObj_city") as GameObject;
    ...
    public IEnumerator RME_cityKounaiEffect()
    {
        Debug.Log("effect設置");
        ...
        GameObject PostProcessVolumeKounaiObj =
            Instantiate(Resources.Load("PostProcessing/Prefab/PostProcessVolume_CityKounaiRoomOn_2nd") as GameObject
            , GirlHeadTrs, false);
        ...
        //Cityでは、SunLightをオフにする仕様（■211031）
        //予めCity_Lightをオフにするようになったので、直接指定する形に変更（■211128）


        

        var faceLightObj = GameObject.Find("GameObjects_City").transform.Find("City_Light/Sun_Realtime")
            .gameObject;
        ...
        //マテリアルを置き換え
        tangSMR.material = tangSMR.GetComponent<MatChangeResources>().otherMatList[0];
        ...
        toothSMR.material = toothSMR.GetComponent<MatChangeResources>().otherMatList[0];
        ...
        kounaiSMR.material = kounaiSMR.GetComponent<MatChangeResources>().otherMatList[0];
        ...
        //■210703追記　LightingSettingのsourceを変更
        RenderSettings.ambientMode = UnityEngine.Rendering.AmbientMode.Flat;

        //初期化終了
        isF3030KounaiEffectInitialize = true;

        Debug.Log("effectループ開始");
    ...
    public IEnumerator RME_cityKounaiEffectOff()
    {
        isF3030KounaiEffect = false;
        yield break;
    }

    public IEnumerator RME_cityLightingSettingSouceFlat()
    {
        //■210815追記　LightingSettingのsourceを変更
        //咥内エフェクトをオフにした後、胃の中に入ってまたSouceをColorにする必要が出たので
        RenderSettings.ambientMode = UnityEngine.Rendering.AmbientMode.Flat;

        //■210815追記　既存のポストプロセスオン（咥内時にオフにしたままになってしまうため）
        PostProcessVolume00DirectDataObj.SetActive(true);

        yield break;
    }

    Tweener RME_cityplayertonderuTweener;
    Tweener RME_citymobtonderuTweener;
    GameObject GbTaichi;
    public IEnumerator RME_CityTonderuPlayerotikake()
    {
        //Tweenで動かしつつ、セリフが終わったら次の動きへ上書き更新

        //モブ取得
        GbTaichi = GameObject.Find("Taichi_400_m");

        //■PosObj読み込み
        //落ちかけPosObj
        //マコト
        GameObject yd_playerPosObj_city2_otikake
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_playerPosObj_city2_otikake") as GameObject;
        //モブ
        GameObject yd_mob_Taichi_charPosObj_city2_otikake
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_mob_Taichi_charPosObj_city2_otikake") as GameObject;


        //吐き出す動きを専用変数に付与
        //マコト
        RME_cityplayertonderuTweener =
            CameraObjectsTrs.DOLocalMove
            (yd_playerPosObj_city2_otikake.transform.localPosition
            , 3)
            .SetEase(Ease.OutQuart);
        //モブ
        RME_citymobtonderuTweener =
            GbTaichi.transform.DOLocalMove
            (yd_mob_Taichi_charPosObj_city2_otikake.transform.localPosition
            , 3)
            .SetEase(Ease.OutQuart);
        ...
        (yd_playerPosObj_city2_otikake.transform.localEulerAngles
        , 2)
        .SetEase(Ease.OutQuart);
    ...
    public IEnumerator RME_CityTonderuPlayerSlowMove()
    {
        //Tweenで動かしつつ、セリフが終わったら次の動きへ上書き更新
        ...
        //ゆっくり動き
        //マコト
        GameObject yd_playerPosObj_city3_sora
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_playerPosObj_city3_sora") as GameObject;
        //モブ
        GameObject yd_mob_Taichi_charPosObj_city3_sora
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_mob_Taichi_charPosObj_city3_sora") as GameObject;

        //吐き出す動きをキャンセル
        //マコト
        RME_cityplayertonderuTweener.Kill();
        //モブ
        RME_citymobtonderuTweener.Kill();

        //ゆっくり動きを専用変数に付与
        //マコト
        RME_cityplayertonderuTweener =
            CameraObjectsTrs.DOLocalMove
            (yd_playerPosObj_city3_sora.transform.localPosition
            , 60)
            .SetEase(Ease.OutSine);
        //モブ
        RME_citymobtonderuTweener =
            GbTaichi.transform.DOLocalMove
            (yd_mob_Taichi_charPosObj_city3_sora.transform.localPosition
            , 60)
            .SetEase(Ease.OutSine);

        yield break;
    }
    public IEnumerator RME_CityTonderuPlayerOtiru()
    {
        //Tweenで動かしつつ、セリフが終わったら次の動きへ上書き更新
        ...
        //落ちる
        //マコト
        GameObject yd_playerPosObj_city4_kounaigoal
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_playerPosObj_city4_kounaigoal") as GameObject;
        //モブ
        GameObject yd_mob_Taichi_charPosObj_city4_goal
            = Resources.Load("_PosObj/D_Henai_Day1/_ResizeMeEventteam/yd_load/yd_mob_Taichi_charPosObj_city4_goal") as GameObject;
        ...
        RME_cityplayertonderuTweener =
            CameraObjectsTrs.DOLocalMove
            (yd_playerPosObj_city4_kounaigoal.transform.localPosition
            , 5)
            .SetEase(Ease.InOutSine);
        //モブ
        RME_citymobtonderuTweener =
            GbTaichi.transform.DOLocalMove
            (yd_mob_Taichi_charPosObj_city4_goal.transform.localPosition
            , 5)
            .SetEase(Ease.InOutSine);
