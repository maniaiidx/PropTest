...
{
    //こちらで追加 SEをミキサーに渡すためにオリジナルのSE再生メソッドを使うため
    DataCounter DC;
    ...
    //コンポーネントバリュー コピペで書き換わってしまって混乱するので、表示するようにした（InspectorのDebug表示にすると出る）
    ...
    //■その時は↓コレがコピペでひとつのObjに指定されていたのが原因だった　↑や、さらに下のは未確認
    ...
    public void Explode(Vector3 v3ExplosionPosition, float fExplosionForce)
    {
        ...
        if (EventExplosionSound)
        {
            ...
            DC.SEPlay(DC.FracturedSEObj, EventExplosionSound, v3ExplosionPosition);
        }
    ...
    public void NotifyDetachChunkCollision(Vector3 v3Position, bool bIsMain)
    {
        if (EventDetachSound != null && bIsMain)
        {
            ...
            DC.SEPlay(DC.FracturedSEObj, EventDetachSound, v3Position);
        }
    ...
    public void NotifyFreeChunkCollision(FracturedChunk.CollisionInfo collisionInfo)
    {
        ...
        if (collisionInfo.bCancelCollisionEvent == false)
        {
            if (EventDetachedSoundArray.Length > 0)
            {
                ...
                //これがnullでエラーでるのでifつけた（nullの理由はFracturedObjectスクリプトがOffでStart処理で初期化されてなかったから）
                if (m_afFreeChunkSoundTimers != null)
                {
                    for (int nSound = 0; nSound < m_afFreeChunkSoundTimers.Length; nSound++)
                    {
                        if (m_afFreeChunkSoundTimers[nSound] <= 0.0f)
                ...
                {
                    Debug.Log("■FracturedObjectスクリプトがOff？ \n m_afFreeChunkSoundTimersがNull \n EventDetachedMaxSounds = " + EventDetachedMaxSounds.ToString());
                }
                ...
                if (nFreeSound != -1)
                {
                    ...
                    if (clip != null)
                    {
                        ...
                        DC.SEPlay(DC.FracturedSEObj, clip, collisionInfo.collisionPoint);
                    }
            ...
            if (EventDetachedPrefabsArray.Length > 0)
            {
                ...
                if (prefab != null)
                {
                    ...
                    //これがnullでエラーでるのでifつけた
                    if (m_afFreeChunkPrefabTimers != null)
                    {
                        for (int nPrefab = 0; nPrefab < m_afFreeChunkPrefabTimers.Length; nPrefab++)
                        {
                            if (m_afFreeChunkPrefabTimers[nPrefab] <= 0.0f)
                    ...
                    {
                        Debug.Log("■FracturedObjectスクリプトがOff？ \n m_afFreeChunkPrefabTimersがNull \n EventDetachedMaxPrefabs = " + EventDetachedMaxPrefabs.ToString());
                    }
    ...
    public void NotifyImpact(Vector3 v3Position)
    {
        if (EventImpactSound != null)
        {
            ...
            DC.SEPlay(DC.FracturedSEObj, EventImpactSound, v3Position);
        }
