...
public class FracturedChunk : MonoBehaviour
{
    ...
    //public Transform custom_DetachParentTrs = null;
    ...
    //こちらで追加 isKinematic操作用（AABBエラー時や、破壊後に処理軽くする時など用）
    ...
    void Update()
    {
        if(Application.isPlaying)
        {
            #region こちらで追加 （AABB（PositionがNaNになった時の対処））
            if (float.IsNaN(transform.position.x)/* || float.IsNaN(transform.position.y) || float.IsNaN(transform.position.z)*/)
            {
                Debug.Log(name+ "にAABB発生（PositionがNaN） isKinematicにして0位置地面に埋めた");
    ...
    {
        //transform.parent      = fracturedObjectSource.transform; //こちらでコメントアウト
    ...
    public void DetachFromObject(bool bCheckStructureIntegrity = true)
    {
        if(IsDestructibleChunk() && IsDetachedChunk == false && GetComponent<Rigidbody>())
        {
            ...
            //transform.parent      = null; //こちらでコメントアウト
