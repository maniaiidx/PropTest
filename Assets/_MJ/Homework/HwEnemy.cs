...
public class HwEnemy : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        //●●●ゲージ表示関係
        //子にスライダー本体を持ったオブジェクト（SliderEnmSet）をHwCanvasに一旦ペアレントして、
        //位置設定したあと本体を敵にペアレントして切り離すことでゲージの場所と大きさ統一をする
        HwCanvas = GameObject.Find("HwCanvas");
        SliderEnmSet = Instantiate(Resources.Load("EventSystem/Homework/SliderEnmSet")) as GameObject;
        SliderEnmSet.transform.SetParent(HwCanvas.transform, false);
    ...
    IEnumerator DeActive(float delay)
    {
        ...
        this.gameObject.SetActive(false);//削除ではなくオフ（復活させるために）
    }
